...
final class ToDoDetailsSheetViewController: UIViewController {
    ...
    private func setupLayout() {
        ...
        addActionRow(title: "Редактировать", systemName: "square.and.pencil", isDestructive: false, action: #selector(editTapped))
        addSeparator()
        addActionRow(title: "Поделиться",   systemName: "square.and.arrow.up", isDestructive: false, action: #selector(shareTapped))
        addSeparator()
        addActionRow(title: "Удалить",       systemName: "trash",               isDestructive: true,  action: #selector(deleteTapped))
    }

    // MARK: Action rows
    private func addActionRow(title: String, systemName: String, isDestructive: Bool, action: Selector) {
        let row = UIControl()
